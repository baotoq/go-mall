...
func (_c *OrderCreate) SetTotalCents(v int64) *OrderCreate {
	_c.mutation.SetTotalCents(v)
	return _c
}

// SetNillableTotalCents sets the "total_cents" field if the given value is not nil.
func (_c *OrderCreate) SetNillableTotalCents(v *int64) *OrderCreate {
	if v != nil {
		_c.SetTotalCents(*v)
	}
	return _c
...
func (_c *OrderCreate) check() error {
	...
	if _, ok := _c.mutation.TotalCents(); !ok {
		return &ValidationError{Name: "total_cents", err: errors.New(`ent: missing required field "Order.total_cents"`)}
	}
	if _, ok := _c.mutation.Currency(); !ok {
