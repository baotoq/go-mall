...
const markDeadSQL = `UPDATE outbox_messages SET retry_count = $2, next_attempt_at = clock_timestamp() + INTERVAL '100 years', last_error = $3 WHERE id = $1`
...
const sweepInboxSQL = `DELETE FROM inbox_states WHERE message_id IN (SELECT message_id FROM inbox_states WHERE received_at < clock_timestamp() - ($1 * INTERVAL '1 second') LIMIT 1000)`
